...
//		to point it out 
//
// 07/11/2013 Armin Kappeler
// - cost values are not saved in list, but taken from an external array
// - Add Mex update-functionality
...
/**
 * This class provides a back-index heap (MinHeap) structure where indexes of 
...
public:
	...
	void push( Tkey key, int index ){
		...
		if( useBackIdx && index >= (int) backIdx.size() )
			...
			//TODO: estend the backIdx-array dynamically
		...
		if( !useBackIdx ){
			...
			heapDecreaseKey( (int)heap.size()-1, key );
		}
		else{
    ...
    /// Armin: checks, if an element is contained in the list
    int checkIdx( int index ){
        if (index<0){
            return -1;
        }
        else{
            return backIdx[index];
        }
